...
export default function NewDispute() {
  ...
  useEffect(() => {
    // Read shop from sessionStorage (saved after OAuth)
    const saved = sessionStorage.getItem('disputeiq_shop') || ''
  ...
  const submit = async () => {
    ...
    if (!shop) { setError('No store connected. Return to dashboard and reconnect your store.'); return }
    ...
    if (res.ok) { setDone(true) } else { setError('Failed to create dispute — check your connection and try again'); setLoading(false) }
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f9fafb', fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif' }}>
      ...
      <div style={{ maxWidth:560, margin:'0 auto', padding:'48px 24px' }}>
        ...
        <p style={{ fontSize:14, color:'#6b7280', marginBottom:8 }}>For disputes that arrived outside the automatic detection window.</p>
        {shop && <p style={{ fontSize:12, color:'#9ca3af', marginBottom:24 }}>Store: {shop}</p>}
        <div style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:14, padding:24, display:'flex', flexDirection:'column', gap:16 }}>
          ...
          <button onClick={submit} disabled={loading}
            style={{ background:loading ? '#86efac' : '#16a34a', color:'#fff', border:'none', borderRadius:10, padding:'13px', fontSize:15, fontWeight:600, cursor:loading ? 'not-allowed' : 'pointer', fontFamily:'inherit' }}>
