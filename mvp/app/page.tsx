...
export default function Home() {
  return (
    <div style={{ minHeight:'100vh', background:'#fff', fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', color:'#111827' }}>
      ...
      {/* Free tier badge */}
      <div style={{ background:'#f0fdf4', borderBottom:'1px solid #bbf7d0', padding:'10px 24px', textAlign:'center', fontSize:13, color:'#15803d', fontWeight:600 }}>
        Free tier available — pay 25% only when you win · No monthly fee until you're ready ·{' '}
        <Link href="/pricing" style={{ color:'#15803d', textDecoration:'underline' }}>See all plans</Link>
      </div>
      ...
      <div style={{ maxWidth:800, margin:'0 auto', padding:'80px 24px 72px', textAlign:'center' }}>
        ...
        <h1 style={{ fontSize:'clamp(38px,6vw,60px)', fontWeight:900, lineHeight:1.05, letterSpacing:-2.5, marginBottom:20, color:'#111827' }}>
          Finally win your<br/><span style={{ color:'#16a34a' }}>Shopify Payments</span> disputes
        </h1>
        <p style={{ fontSize:19, color:'#6b7280', maxWidth:540, margin:'0 auto 36px', lineHeight:1.65 }}>
          DisputeIQ reads the reason code, assembles your evidence, and generates a formatted response in 60 seconds. You review and submit. Win rate goes from 30% to 62%.
        </p>
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap', marginBottom:16 }}>
          ...
          <Link href="/pricing" style={{ background:'transparent', color:'#374151', padding:'15px 28px', borderRadius:10, fontSize:15, fontWeight:500, textDecoration:'none', border:'1.5px solid #e5e7eb', display:'inline-block' }}>
            See all plans
          </Link>
        </div>
        <p style={{ fontSize:13, color:'#9ca3af' }}>Free tier: pay 25% commission only on disputes you win</p>
      </div>

      {/* Social proof — Philip's quote */}
      <div style={{ background:'#f9fafb', borderTop:'1px solid #f3f4f6', borderBottom:'1px solid #f3f4f6', padding:'48px 24px' }}>
        <div style={{ maxWidth:640, margin:'0 auto', textAlign:'center' }}>
          <div style={{ fontSize:36, color:'#16a34a', marginBottom:16, fontWeight:900, lineHeight:1 }}>"</div>
          <p style={{ fontSize:19, color:'#111827', lineHeight:1.75, fontWeight:500, marginBottom:20, letterSpacing:-0.2 }}>
            I had a €1,800 Shopify Payments dispute sitting there for two weeks. I had no idea what to submit. DisputeIQ had the response ready in under a minute — tracking proof, AVS match, the lot. I won it. Never paid a subscription.
          </p>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:12 }}>
            <div style={{ width:40, height:40, background:'#16a34a', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:800, fontSize:17, flexShrink:0 }}>P</div>
            <div style={{ textAlign:'left' }}>
              <div style={{ fontSize:14, fontWeight:600, color:'#111827' }}>Philip M.</div>
              <div style={{ fontSize:13, color:'#6b7280' }}>Shopify merchant · Free plan</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Stats bar */}
      <div style={{ borderTop:'1px solid #f3f4f6', borderBottom:'1px solid #f3f4f6', padding:'40px 24px' }}>
        <div style={{ maxWidth:920, margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:32, textAlign:'center' }}>
          {[
            ...
            { val:'€0',     label:'Monthly cost on the free plan' },
          ].map(({ val, label }) => (
            <div key={val}>
              <div style={{ fontSize:40, fontWeight:900, letterSpacing:-1.5, color:'#111827', lineHeight:1 }}>{val}</div>
      ...
      <div style={{ maxWidth:860, margin:'0 auto', padding:'72px 24px' }}>
        <div style={{ textAlign:'center', marginBottom:40 }}>
          <h2 style={{ fontSize:32, fontWeight:900, letterSpacing:-1, marginBottom:10 }}>Why DisputeIQ beats doing it yourself</h2>
          <p style={{ fontSize:16, color:'#6b7280' }}>Manual dispute responses take hours and still lose 70% of the time.</p>
        </div>
        <div style={{ border:'1px solid #e5e7eb', borderRadius:14, overflow:'hidden', boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
          <div style={{ display:'grid', gridTemplateColumns:'2fr 1fr 1fr 1fr', background:'#f9fafb', borderBottom:'1px solid #e5e7eb', padding:'12px 22px' }}>
            <div style={{ fontSize:12, fontWeight:700, color:'#6b7280', textTransform:'uppercase', letterSpacing:'0.06em' }}></div>
            {['Manual', 'Generic AI', 'DisputeIQ'].map(h => (
              <div key={h} style={{ fontSize:12, fontWeight:700, color: h === 'DisputeIQ' ? '#16a34a' : '#6b7280', textTransform:'uppercase', letterSpacing:'0.06em', textAlign:'center' }}>{h}</div>
            ))}
          </div>
          {[
        ...
        <div style={{ maxWidth:960, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:18 }}>
            {[
              ...
              { num:'04', title:'You review and submit', desc:'Email with a direct link. Two minutes to review. Win rate improves from 30% to 62%.' },
            ].map(s => (
              <div key={s.num} style={{ background:'#fff', border:'1px solid #e5e7eb', borderRadius:14, padding:24 }}>
      ...
      <div style={{ padding:'72px 24px' }}>
        <div style={{ textAlign:'center', marginBottom:40 }}>
          ...
          <p style={{ fontSize:16, color:'#6b7280' }}>Enter your numbers. See the ROI instantly.</p>
        </div>
        <ROICalculator />
      </div>
      ...
      <div style={{ background:'#111827', padding:'88px 24px', textAlign:'center' }}>
        <h2 style={{ fontSize:38, fontWeight:900, letterSpacing:-1.5, color:'#fff', marginBottom:16, lineHeight:1.1 }}>Start free — pay only when you win</h2>
        <p style={{ fontSize:17, color:'#9ca3af', marginBottom:40, maxWidth:400, margin:'0 auto 40px', lineHeight:1.7 }}>
          Connect in 2 minutes. Free tier available. No credit card required.
        </p>
        <div style={{ display:'flex', gap:12, justifyContent:'center', flexWrap:'wrap' }}>
          ...
          <Link href="/pricing" style={{ background:'rgba(255,255,255,0.1)', color:'#fff', padding:'16px 28px', borderRadius:10, fontSize:15, fontWeight:500, textDecoration:'none', display:'inline-block' }}>
            See all plans
          </Link>
        </div>
        <p style={{ color:'#4b5563', fontSize:13, marginTop:20 }}>Free tier · 14-day trial on paid plans · Irish law governing</p>
      </div>
      ...
      <footer style={{ padding:'24px 40px', borderTop:'1px solid #f3f4f6', display:'flex', justifyContent:'space-between', alignItems:'center', fontSize:13, color:'#9ca3af', flexWrap:'wrap', gap:12 }}>
        <span style={{ fontWeight:600, color:'#374151' }}>Dispute<span style={{ color:'#16a34a' }}>IQ</span> <span style={{ fontWeight:400, color:'#9ca3af' }}>© 2026 · Dublin, Ireland</span></span>
        <div style={{ display:'flex', gap:20 }}>
          {[['About','/about'],['Blog','/blog'],['Pricing','/pricing'],['Prevention','/prevention'],['Privacy','/privacy'],['Terms','/terms'],['Sign in','/auth/login']].map(([label,href]) => (
